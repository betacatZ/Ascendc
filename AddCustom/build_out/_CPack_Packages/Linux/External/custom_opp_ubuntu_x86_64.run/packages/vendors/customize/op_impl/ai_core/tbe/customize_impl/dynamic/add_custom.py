...
def _build_args(x_in__, y_in__, z_out_, bias):
    __inputs__ = []
...
def add_custom(x_in__, y_in__, z_out_, bias, kernel_name="add_custom", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_out_, bias)
    ...
    print("start compile Ascend C operator AddCustom. kernel name is " + kernel_name)
    op_type = "AddCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, y_in__], origin_outputs = [z_out_], param_type_dynamic = False,\
        mc2_ctx = None, param_type_list = ['required', 'required', 'required'])
