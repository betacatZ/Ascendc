...
function(opbuild)
  ...
  if (NOT EXISTS ${DDK_PATH}/ddk/tbe/set_tbe_env_ddk.sh)
    set(platform_compute_unit "")
    foreach(compute_unit ${ASCEND_COMPUTE_UNIT})
      set(platform_compute_unit "${compute_unit}")
      if (NOT EXISTS ${DDK_PATH}/tools/platform/${compute_unit})
        message(FATAL_ERROR "${DDK_PATH}/tools/platform/${compute_unit} not exists, please install platform plugin first!")
      endif()
    endforeach()
    set(ENV{LD_LIBRARY_PATH} $ENV{LD_LIBRARY_PATH}:${DDK_PATH}/tools/platform/${platform_compute_unit}/lib64)
  endif ()
  ...
  execute_process(COMMAND ${CMAKE_COMPILE} -g -fPIC -shared -std=c++11 ${OPBUILD_OPS_SRC}
                  ...
                  -L ${HIAI_VERSION}/lib64/
                  -L ${DDK_PATH}/ddk/tbe/lib64/
                  -L ${DDK_PATH}/tools/platform/${platform_compute_unit}/lib64/
...
function(build_host)
  ...
  foreach(CUSTOM_OP_PROJ ${CUSTOM_OP_PATH})
      file(GLOB CUSTOM_OP_HOST_SRC ${CUSTOM_OP_PATH}/op_host/*.c*)
      list(APPEND LIB_CUSTOM_OP_SRC ${CUSTOM_OP_HOST_SRC})
  endforeach()

  foreach(CUSTOM_OP_PROJ ${CUSTOM_OP_PATH})
      file(GLOB CUSTOM_OP_HOST_PLUGIN_SRC ${CUSTOM_OP_PATH}/framework/*/*plugin.c*)
  ...
  #custom_op
  if (NOT EXISTS ${DDK_PATH}/ddk/tbe/set_tbe_env_ddk.sh)
    install(TARGETS custom_op DESTINATION ${DDK_PATH}/tools/platform/${platform_compute_unit}/lib64)
    install(FILES ${ASCEND_AUTOGEN_PATH}/op_proto.h DESTINATION ${DDK_PATH}/tools/platform/${platform_compute_unit}/ops/impl/custom/)
  else()
...
function(install_ddk)
  set(LIB_CUSTOM_OP_SRC "")
  foreach(CUSTOM_OP_PROJ ${CMAKE_CURRENT_SOURCE_DIR})
      file(GLOB CUSTOM_OP_HOST_SRC ${CMAKE_CURRENT_SOURCE_DIR}/op_host/*.c*)
      list(APPEND LIB_CUSTOM_OP_SRC ${CUSTOM_OP_HOST_SRC})
  endforeach()
  ...
  foreach(compute_unit ${ASCEND_COMPUTE_UNIT})
    if (NOT EXISTS ${DDK_PATH}/ddk/tbe/set_tbe_env_ddk.sh)
      set(omg_install_path ${DDK_PATH}/tools/platform/${compute_unit}/ops/impl/custom)
      execute_process(
        COMMAND mkdir -p ${omg_install_path}
        COMMAND rm -rf ${omg_install_path}/*
        COMMAND mkdir -p ${DDK_PATH}/tools/platform/${compute_unit}/simulator/model/conf
        COMMAND mkdir -p ${DDK_PATH}/tools/platform/${compute_unit}/simulator/camodel_log
      )
    else()
    ...
    foreach(CUSTOM_OP_PROJ ${CMAKE_CURRENT_SOURCE_DIR})
      ...
      if (NOT EXISTS ${DDK_PATH}/ddk/tbe/set_tbe_env_ddk.sh)
        install(FILES ${custom_py} DESTINATION ${omg_install_path})
        install(FILES ${custom_cpp} DESTINATION ${omg_install_path})
        install(FILES ${custom_h} DESTINATION ${omg_install_path})
      else()
    ...
    if (NOT EXISTS ${DDK_PATH}/ddk/tbe/set_tbe_env_ddk.sh)
      install(FILES ${CMAKE_CURRENT_SOURCE_DIR}/build_out/npu_custom_opinfo.json DESTINATION ${DDK_PATH}/tools/platform/${compute_unit}/config)
      install(FILES ${CMAKE_CURRENT_SOURCE_DIR}/build_out/_CPack_Packages/Linux/External/custom_opp_ubuntu_x86_64.run/packages/vendors/customize/op_impl/ai_core/tbe/config/${compute_unit}/aic-${compute_unit}-ops-info.json DESTINATION ${DDK_PATH}/tools/platform/${compute_unit}/config)
    else()
      ...
      install(FILES ${CMAKE_CURRENT_SOURCE_DIR}/build_out/npu_custom_opinfo.json DESTINATION ${HIAI_VERSION}/lib64/config)
