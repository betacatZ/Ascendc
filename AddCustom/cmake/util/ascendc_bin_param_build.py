...
class BinParamBuilder(opdesc_parser.OpDesc):
    ...
    def _write_buld_cmd(self: any, param_file: str, bin_file: str, index: int):
        ...
        with os.fdopen(os.open(compile_file, const_var.WFLAGS, const_var.WMODES), 'w') as fd:
            ...
            ddk_path = os.getenv('DDK_PATH')
            if not os.path.exists(os.path.join(ddk_path, "ddk", "tbe", "set_tbe_env_ddk.sh")):
                fd.write('export PYTHONPATH=$PYTHONPATH:' + ddk_path + f'/tools/platform/{hard_soc.lower()}/ops/impl\n')
                fd.write('export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:' + ddk_path + f'/tools/platform/{hard_soc.lower()}/lib64:' + ddk_path + f'/tools/platform/{hard_soc.lower()}/simulator\n')
